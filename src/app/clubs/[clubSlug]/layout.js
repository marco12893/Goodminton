...
export default async function ClubLayout({ children, params }) {
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-[#07131f] text-white">
      ...
      <div className="relative mx-auto flex min-h-screen w-full max-w-md flex-col px-4 pb-0 pt-4">
        <header className="pt-2">
          <div className="flex items-center justify-between gap-4">
            <Link
              href="/"
              className="inline-flex items-center rounded-full bg-white/8 px-4 py-2 text-sm font-medium text-white/92 backdrop-blur-xl"
            >
              Back
            </Link>
            <div className="min-w-0 text-right">
              <p className="truncate font-mono text-[2rem] font-semibold leading-none text-white">
                {club.name}
              </p>
              <p className="mt-2 text-sm text-white/65">{club.city || "Venue belum diisi"}</p>
            </div>
          </div>
        </header>

        <div className="flex flex-1 flex-col gap-5 pb-4 pt-6">{children}</div>
